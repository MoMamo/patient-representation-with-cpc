...
class TruncateSeqence(object):
    ...
    def __call__(self, sample):
        sample.update({
            ...
            'len': len(sample['code'][-self.max_seq_length:])
...
class checkChronicConditions(object):
    def __init__(self, code_map):
        self.code_map = code_map

    def __call__(self, sample):
        def check_exist(x, ref, mordality):
            if mordality == 'DIA':
                ref = ['DIA' + each for each in ref]
            elif mordality == 'MED':
                ref = ['MED' + each for each in ref]
        ...
        for condition, value in self.code_map.items():
            check_condition[condition] = check_exist(code, ref=value['code'], mordality=value.get('type', 'DIA'))
        sample.update(check_condition)
        return sample
